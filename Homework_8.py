...
def test_1():
    ...
    driver.find_element_by_name("campaigns[new_1][start_date]").send_keys(datetime.date.today().strftime('%d-%m-%Y')+(Keys.ARROW_RIGHT)+(datetime.datetime.now().strftime('%H:%M')))
    driver.find_element_by_name("campaigns[new_1][end_date]").send_keys((datetime.date.today()+datetime.timedelta(days=30)).strftime('%d-%m-%Y')+(Keys.ARROW_RIGHT)+(datetime.datetime.now().strftime('%H:%M')))
